...
bool check(SetIterator iter, taco::TensorBase& tensor, bool checkShape=false) {
    ...
    while(iter->hasNextRecord()) {
        ...
        if(checkShape) {
            ...
            std::cout << "sizes: " << answerFormat.size() << ", " << copyFormat.size() << std::endl;
            for(int mode = 0; mode != copyFormat.size(); ++mode) {
                std::cout << "checkShape " << mode << ":  " << copyFormat[mode] << ", " << answerFormat[mode] << std::endl;
...
int main() {
    ...
    std::cout << "********************************************************" << std::endl;
    //pdbClient.listAllRegisteredMetadata();
